...
func cmdBoot(cfg Config) {
	...
	for _, name := range bootOrder {
		...
		current := loadCurrentState(cfg.StateDir, name)
		if current == nil {
			resetStaleBackoffOnBoot(cfg, name, current)
			continue
		}
...
// resetStaleBackoffOnBoot clears a persisted activation backoff for an overlay
// that has no committed version at boot. The manager's state dir is persistent,
// but an overlay's placement — generated units and /run/overlays symlinks —
// lives on tmpfs and is wiped by a reboot. So for an uncommitted overlay
// (current == nil) the last recorded attempt describes services that no longer
// exist; keeping its backoff would skip re-activation for hours and leave the
// overlay absent across every reboot (the failure mode that wedged cofferd on
// lordhenry). Clearing it lets the first post-boot check retry from a clean
// slate; within-boot backoff still applies the moment a fresh attempt fails.
//
// A committed overlay (current != nil) keeps its backoff: its good version was
// regenerated by cmdBoot and is running, and a failing newer version must not
// thrash it. Returns true if a stale record was cleared.
func resetStaleBackoffOnBoot(cfg Config, name string, current *OverlayState) bool {
	if current != nil {
		return false
	}
	attempt := loadAttempt(cfg.StateDir, name)
	if attempt == nil {
		return false
	}
	log.Printf("[%s] boot: no committed version; clearing stale activation backoff (was x%d, %s: %s)",
		name, attempt.Attempts, attempt.State, attempt.Reason)
	clearAttempt(filepath.Join(cfg.StateDir, name))
	return true
}
