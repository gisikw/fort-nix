...
### Phase 1: Consumer Callback Infrastructure
...
**1.5 Path consolidation**
- Build-time configs: `/etc/fort/` (not `/etc/fort-agent/`)
- Runtime state: `/var/lib/fort/` (consolidate `/var/lib/fort-agent/`)
...
**2.2 Add provider state management**
...
**2.3 Implement async handler invocation**
...
**2.4 Implement callback dispatch**
...
**4.1 Git tokens (first - already partially working)**
...
```
Phase 1.1 (callback endpoint) ←─ Phase 2.4 (callback dispatch)
Phase 1.2 (needs enum) ←─ Phase 3.1 (GC sweep)
Phase 1.3 (fulfillment state) ←─ Phase 1.1 (callback endpoint)
Phase 2.2 (provider state) ←─ Phase 2.3 (async invocation)
Phase 2.3 (async invocation) ←─ Phase 2.4 (callback dispatch)
Phase 2.* ←─ Phase 4.* (migrations)
Phase 3.* ←─ Phase 4.* (can do migrations before GC, but GC needed for cleanup)
```

Suggested order: 1.2 → 1.3 → 1.1 → 1.4 → 2.2 → 2.1 → 2.3 → 2.4 → 2.5 → 2.6 → 3.* → 4.*

## Estimated Scope

| Phase | Effort | Risk |
|-------|--------|------|
| 1.1 | Medium | Low - extend existing wrapper |
| 1.2 | Small | Low - simple endpoint |
| 1.3 | Small | Low - add state file |
| 1.4 | Small | Low - extend options |
| 2.1-2.2 | Medium | Medium - new state management |
| 2.3 | Large | Medium - major handler change |
| 2.4 | Medium | Medium - callback dispatch |
| 2.5-2.6 | Medium | Low - systemd wiring |
| 3.* | Medium | Medium - GC correctness |
| 4.* | Variable | Medium - per-service migration |

Phase 1 is low-risk incremental work. Phase 2 is the major architectural addition.
Phase 3 is important for long-term health. Phase 4 is payoff.
