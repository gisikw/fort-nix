...
if (deviceProfileManifest.platform or "nixos") != "nixos" then
  ...
  let
    ...
    herdr = import ../../pkgs/herdr { inherit pkgs; };
    ...
    principalsWithDevSandbox = builtins.filter (p: builtins.elem "dev-sandbox" (p.roles or [ ])) (
  ...
  in
  {
    ...
    environment.etc."fort-git-credential-helper".source =
