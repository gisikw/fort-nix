  ...
  systemd.services.cupola = {
    ...
    serviceConfig = {
      ...
      Environment = [
        ...
        "MIX_ENV=dev"
        "PORT=${toString port}"
        "PATH=${pkgs.lib.makeBinPath (with pkgs; [ elixir erlang git coreutils bash esbuild tailwindcss_4 ])}:/run/overlays/bin:/run/managed-bin:/home/dev/.local/bin:/run/current-system/sw/bin"
