...
Replace with automatic enrollment: a launchd one-shot that runs on first boot (or activation) and enrolls with headscale if not already registered. Guard on something like `tailscale status` exit code — if already connected, no-op.
